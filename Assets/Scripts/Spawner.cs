...
public class Spawner : MonoBehaviour
{
    ...
    /// </summary>
    public float spawnFrequencyMean = 5f;
    
    /// <summary>
    /// Standard deviation of the frequency of spawning as n per second.
    /// </summary>
    public float spawnFrequencyStd = 0.0f;
    ...
    public void SpawnObstacle()
    {
        ...
        var randomHeight = Random.Range(spawnSize , spawnSize + 1f);

        // Move it to the target location.
        var spawnDown = RandomBool();
        obstacle.transform.position += (Vector3)(spawnDown ? 
            spawnOffset + (1.0f - randomHeight) / 2.0f : 
            -spawnOffset - (1.0f - randomHeight) / 2.0f
        );
        
        // Scale it.
        obstacle.transform.localScale = new Vector3(spawnSize, randomHeight, spawnSize);
        
        // Move the obstacle into the correct layer.
        obstacle.layer = LayerMask.NameToLayer(spawnLayer);

        //Choose material at random
        var mask = obstacle.transform.GetChild(0).GetComponent<SpriteRenderer>();
        var sprite = obstacle.transform.GetChild(1).GetComponent<SpriteRenderer>();
        sprite.sprite = randomSprites[Random.Range(0,randomSprites.Length)];
        mask.sprite = randomMasks[Random.Range(0,randomMasks.Length)];
    }
    ...
    public void ModifyObstacleSpeed(float multiplier)
    {
        ...
        var xMultiplier = new Vector2(multiplier, 2.0f);
        foreach (Transform child in transform)
